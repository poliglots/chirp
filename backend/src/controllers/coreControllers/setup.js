...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      chirp_app_email: email,
      chirp_app_company_email: email,
      chirp_app_timezone: timezone,
      chirp_app_country: country,
      chirp_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully chirp App Setup',
  });
};
